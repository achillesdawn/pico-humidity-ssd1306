...
void calc_render_area_buflen(struct RenderArea *area) {
...
void render(uint8_t *buf, struct RenderArea *area) {
...
static void SetPixel(uint8_t *buf, int x, int y, bool on) {
    ...
    // The calculation to determine the correct bit to set depends on which
    // address mode we are in. This code assumes horizontal

    // The video ram on the SSD1306 is split up in to 8 rows, one bit per pixel.
    // Each row is 128 long by 8 pixels high, each byte vertically arranged, so
    // byte 0 is x=0, y=0->7, byte 1 is x = 1, y=0->7 etc

    // This code could be optimised, but is like this for clarity. The compiler
    // should do a half decent job optimising it anyway.

    const int BytesPerRow = SSD1306_WIDTH; // x pixels, 1bpp, but each row is 8
                                           // pixel high, so (x / 8) * 8
...
static void FillReversedCache() {
    // calculate and cache a reversed version of fhe font, because I defined it
    // upside down...doh!
